...
double interpolate(double x, double y, triDvec &f , twoDvec &A , int t);
...
//Time parameters
double dt = 0.001 , steady_T = 1.0 , T = 250+steady_T;
int n_time_steps = (int)((T+steady_T)/dt);
int t_step;

//Flow parameters
const int nx = 4000, ny = 400;
const double  LX = 10.0 , gridspace_x = LX/(double)nx;
const double LY = 1.0 , gridspace_y = LY/(double)ny;
...
const double stddev = 1.0;//(double)n_fields;
const double trans_noise = 0.0;//sqrt(2.0*dt*0.0001);
const double rot_noise = 0.0;//sqrt(2.0*dt*0.0005) ;
...
int main(){
  ...
  int state , action , state_new , episodes_number = 3000 , heaviside;
  ...
  double init_x , last_x , delta_x , reward_sum , trSS_sum;
  ...
  for(int i=1 ; i <= n_fields ; ++i){
    ...
    UXS.push_back(load_field(path, "/flowData/uxs", nx , ny));
    UYS.push_back(load_field(path, "/flowData/uys", nx , ny));
    ...
    D2UX12S.push_back(load_field(path, "/flowData/d2ux12s", nx , ny));
    D2UX22S.push_back(load_field(path, "/flowData/d2ux22s", nx , ny));
    D2UY11S.push_back(load_field(path, "/flowData/d2uy11s", nx , ny));
  ...
  for(int episode=0 ; episode < episodes_number ; episode++)
    {
      ...
      ux = interpolate(xmod,ymod,UXS,A,t_step);
      uy = interpolate(xmod,ymod,UYS,A,t_step);
      a11 = interpolate(xmod,ymod,A11S,A,t_step);
      a12 = interpolate(xmod,ymod,A12S,A,t_step);
      a21 = interpolate(xmod,ymod,A21S,A,t_step);
      a22 = -a11;
      d2ux11 = interpolate(xmod,ymod,D2UX11S,A,t_step);
      d2ux12 = interpolate(xmod,ymod,D2UX12S,A,t_step);
      d2ux22 = interpolate(xmod,ymod,D2UX22S,A,t_step);
      d2uy11 = interpolate(xmod,ymod,D2UY11S,A,t_step);
      d2uy12 = -d2ux11;
      d2uy22 = -d2ux12;
      ...
      trSS = 2.0*a11*a11 + 0.5*(a12+a21)*(a12+a21);
      state = GET_STATE((gradx*cos(phi_0)+grady*sin(phi_0)) , trSS , -sin(phi_0));
      state_new = state;
      ...
      while(t < T)
	{
	  while(state_new == state)
	    {
	      vx = ux + v_s*cos(phi_0) - vg_ortho + (vg_ortho - vg_para)*(cos(phi_0)*cos(phi_0));
	      vy = uy + v_s*sin(phi_0) + (vg_ortho - vg_para)*(sin(phi_0)*cos(phi_0));
	      
	      x = x_0 + dt*vx;
	      y = y_0 + dt*vy;
	      phi = phi_0 + dt*(omega_flow/2.0 + omega_s*K);
	      ...
	      grady = 4.0*a11*d2ux12 + (a12 + a21)*(d2ux22 + d2uy12);
	      trSS = 2.0*a11*a11 + 0.5*(a12+a21)*(a12+a21);
	      state_new = GET_STATE((gradx*cos(phi)+grady*sin(phi)), trSS , -sin(phi));
	      trSS_sum += trSS;
	      t += dt;
	      t_step++;
	      if(t>T){break;}
	  ...
	  R = (beta*heaviside + 2.0*(1.0 - beta)*sign(delta_x));
	  if(t > steady_T)
	    {
	      // update the Q-matrix
      ...
      delta_y_list.push_back((x - init_x)/(vs*T)); // to keep track of the total vertical migration per episode
...
twoDvec generate_As(double sigma , double tau , int n_time_steps , double dt)
{
  twoDvec A = twoDzeros(n_fields, (n_time_steps));
  //initializing As with Gaussian independent random values with std sigma
  for (int i = 0; i < n_fields; ++i)
    {
      A[i][0] = sqrt(sigma)*dist(generator);
    }
  ...
  for (int i = 0; i < n_fields; ++i)
      ...
      for (int indx = 0; indx < nx; ++indx)
	{
	  for (int indy = 0; indy < ny; ++indy)
	    {
	      v[indx][indy] += V[i][indx][indy]*A[i][t];
	    }
	}
    }
  return v;
...
{    
  ...
  for (int i = 0; i < n_fields; ++i)
    {
      g00 += f[i][y0_ind][x0_ind]*A[i][t];
      g10 += f[i][y1_ind][x0_ind]*A[i][t];
      g01 += f[i][y0_ind][x1_ind]*A[i][t];
      g11 += f[i][y1_ind][x1_ind]*A[i][t];
    }
