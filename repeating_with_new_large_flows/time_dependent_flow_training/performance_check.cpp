...
//Time parameters
double dt = 0.01 , steady_T = 10.0 , T = 1000+steady_T;
...
const int steady_A = (int)(tau*10/dt);
...
const double omega_s = eta/(sqrt(2.0+2.0)*u_rms), vs = PHI*u_rms ;
double v_s;
...
int main(){
  ...
  int state , action , state_new , episodes_number = 10000 , heaviside;
  ...
  for(int i=1 ; i <= n_fields ; ++i){
    ...
    UXS.push_back(load_field(path, "/flowData/uys", nx , ny)); // uys is given as UXS because of transposing
    UYS.push_back(load_field(path, "/flowData/uxs", nx , ny)); // uxs is given as UYS because of transposing
