...
//Time parameters
double dt = 0.001 , steady_T = 1.0 , T = 50+steady_T;
...
const double trans_noise = sqrt(2.0*dt*0.000001);
const double rot_noise = sqrt(2.0*dt*0.0005) ;
...
int main(){
  ...
  double init_y , last_y , delta_y , reward_sum , episode_trSS_sum , update_trSS_sum;
  ...
  for(int episode=0 ; episode < episodes_number ; episode++)
    {
      A.clear();
      A = generate_As(sigma , tau , n_time_steps , dt); //choosing one of the flows randomly
      ...
      while(t < T)
	{
	  ...
	  //Calculating reward
	  delta_y = y - last_y;
	  R = (1.0 - beta)*(delta_y/t_update) - (beta)*(update_trSS_sum/t_update);
	  if(t > steady_T)
	    {
	      // update the Q-matrix
...
{    
  ...
  for (int i = 0; i < n_fields; ++i)
    {
      g00 += f[i][y0_ind][x0_ind]*A[i][t+steady_A];
      g10 += f[i][y1_ind][x0_ind]*A[i][t+steady_A];
      g01 += f[i][y0_ind][x1_ind]*A[i][t+steady_A];
      g11 += f[i][y1_ind][x1_ind]*A[i][t+steady_A];
    }
