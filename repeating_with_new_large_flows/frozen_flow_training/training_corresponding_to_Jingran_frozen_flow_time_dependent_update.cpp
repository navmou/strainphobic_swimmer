/***
    ...
    Jun 15, 2022
    Changed the state transition to be based on n_times_steps.
    Changed the reward to be continuous. (1-\beta)*(\delta y/ \delta t) - \beta*(\sum trSS / \delta t)
    where \delta t is the update time
    made the time_step size dt smaller (10^-3)
    Decrease the training time to be 50 sec., and steady_T to be 1 sec.
    Decreased \gamma to be 0.99    

    Jun. 17, 2022
    Changed the gravity direction to -x to use the flow data without transposing
    To prevent any problems in evaluating the correct values of the flow.
    ...
    - Consider a number of swimming velocities, for example v_s={0.1,0.2,0.5,1,2,5,10} (or enlarge this range if it seems to be of relevance).
***/
...
//Time parameters
double dt = 0.001 , steady_T = 1.0 , T = 200+steady_T;
...
int field_ind;
const double tauK = eta/(2.0*u_rms); 
...
int main(){
  ...
  double init_x , last_x , delta_x , reward_sum , episode_trSS_sum , update_trSS_sum;
  ...
  for(int episode=0 ; episode < episodes_number ; episode++)
    {
      ...
      reward_sum = 0.0; episode_trSS_sum = 0.0;
      ...
      state = GET_STATE(sqrt(2.0*trSS)*tauK);
      state_new = state;
	...
	{
	  update_trSS_sum = 0.0;
	  for(int update=0;update<t_update;++update)
	    {
	      vx = ux + v_s*cos(phi_0);
	      vy = uy + v_s*sin(phi_0);
	      ...
	      shape = Lambda*((cos(phi_0)*cos(phi_0)-sin(phi_0)*sin(phi_0))*(0.5*(a12+a21)) - a11*cos(phi_0)*sin(phi_0));
	      ...
	      episode_trSS_sum += trSS;
	      update_trSS_sum += trSS;
	      t += dt;
	      if(t>T){break;}
	    }
	  
	  state_new = GET_STATE(sqrt(2.0*trSS)*tauK);
	  //Calculating reward
	  delta_x = x - last_x;
	  R = -1.0*trSS*tauK*tauK;
	  if(t > steady_T)
	    {
	      // update the Q-matrix
      ...
      trSS_list.push_back(episode_trSS_sum/(double)n_time_steps);
