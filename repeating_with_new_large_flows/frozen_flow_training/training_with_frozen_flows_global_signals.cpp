...
int GET_STATE(double uy , double grady_ss , double trSS);
...
//Time parameters
double dt = 0.01 , steady_T = 100.0 , T = 2000+steady_T;
	  ...
	  while(state_new == state)
	    {
	      ...
	      grady = 2*a11*d2ux12 + 2*a22*d2uy22 + (a12 + a21)*(d2ux22 + d2uy12);
	      trSS = a11*a11 + 0.5*(a12+a21)*(a12+a21) + a22*a22;
	      state_new = GET_STATE(uy , grady , trSS);
	      trSS_sum += trSS;
	      t += dt;
	      t_step++;
	      if(t>T){break;}
...
//STATE EVALUATION
int GET_STATE(double uy , double grad_sy , double TrSS){
  int state;
  if(TrSS < trSS_threshold)
    {
      if(grad_sy < -grad_threshold)
	{
	  if(uy < -uy_threshold){state = 0;}
	  else if(uy > uy_threshold){state = 2;}
	  else{state = 1;}
	}
      else if(grad_sy > grad_threshold)
	{
	  if(uy < -uy_threshold){state = 6;}
	  else if(uy > uy_threshold){state = 8;}
	  else{state = 7;}
	}
      else
	{
	  if(uy < -uy_threshold){state = 3;}
	  else if(uy > uy_threshold){state = 5;}
	  else{state = 4;}
	}
    }
  else
    {
      if(grad_sy < -grad_threshold)
	{
	  if(uy < -uy_threshold){state = 9;}
	  else if(uy > uy_threshold){state = 11;}
	  else{state = 10;}
	}
      else if(grad_sy > grad_threshold)
	{
	  if(uy < -uy_threshold){state = 15;}
	  else if(uy > uy_threshold){state = 17;}
	  else{state = 16;}
	}
      else
	{
	  if(uy < -uy_threshold){state = 12;}
	  else if(uy > uy_threshold){state = 14;}
	  else{state = 13;}
	}
    }
  return state;
