/***
    ...
    Mar. 16, 2022
    Using only signals \nabla TrSS in local frame:
    \nabla TrSS \cdot \hat{\ve n} 
...
int GET_STATE(double local_grad);
...
//Time parameters
double dt = 0.01 , steady_T = 50.0 , T = 500+steady_T;
...
const int field_x = 1024, field_y = 1024;
...
int main(){
  ...
  twoDvec UX, UY, A11, A12, A21 , A22;
  twoDvec D2UX11 , D2UX12 , D2UX22 , D2UY11 , D2UY12 , D2UY22;
  double a11 , a12 , a21 , a22;
  double d2ux11 , d2ux12 , d2ux22 , d2uy11 , d2uy12 , d2uy22;

  //defining the variables
  int state , action , state_new , field_index , episodes_number = 1000 , heaviside;
  ...
  for(int i=0 ; i < (int)fields_list.size() ; ++i)
    {
      ...
      log_file << "Field " << i+1 << " is loaded" << endl;
    }
  ...
  /********************** episode loop **********************/
  log_file << "Enterign the training Episodes" << endl;
  for(int episode=0 ; episode < episodes_number ; episode++)
    {
      //Producing the new flow
      UX.clear(); UY.clear(); A11.clear(); A12.clear(); A21.clear(); A22.clear();
      D2UX11.clear();D2UX12.clear();D2UX22.clear();D2UY11.clear();D2UY12.clear();D2UY22.clear();
      UX = update_flow(UXS , A , t); UY = update_flow(UYS , A , t);
      A11 = update_flow(A11S , A , t); A21 = update_flow(A21S, A , t);
      A12 = update_flow(A12S , A , t); A22 = update_flow(A11S , A , t);
      D2UX11 = update_flow(D2UX11S,A,t); D2UX12 = update_flow(D2UX12S,A,t); D2UX22 = update_flow(D2UX22S,A,t);
      D2UY11 = update_flow(D2UY11S,A,t); D2UY12 = update_flow(D2UY12S,A,t); D2UY22 = update_flow(D2UY22S,A,t);

      log_file << "Field updated episode " << episode << endl;
      ...
      //interpolation of the fields in the current position
      xmod = get_mod(x_0, L0) , ymod = get_mod(y_0, L0);
      a11 = interpolate(xmod,ymod,A11);
      a12 = interpolate(xmod,ymod,A12);
      a21 = interpolate(xmod,ymod,A21);
      a22 = interpolate(xmod,ymod,A22);
      ux = interpolate(xmod,ymod,UX);
      uy = interpolate(xmod,ymod,UY);
      d2ux11 = interpolate(xmod,ymod,D2UX11);
      d2ux12 = interpolate(xmod,ymod,D2UX12);
      d2ux22 = interpolate(xmod,ymod,D2UX22);
      d2uy11 = interpolate(xmod,ymod,D2UY11);
      d2uy12 = interpolate(xmod,ymod,D2UY12);
      d2uy22 = interpolate(xmod,ymod,D2UY22);
      ...
      state = GET_STATE((gradx*cos(phi_0)+grady*sin(phi_0)));
      state_new = state;
	  ...
	  while(state_new == state)
	    {
	      ...
	      log_file << "update field in episode " << episode << " is done in : " << elapsed_episode.count() << " s\n";
	      ...
	      grady = 2*a11*d2ux12 + 2*a22*d2uy22 + (a12 + a21)*(d2ux22 + d2uy12);
	      trSS = a11*a11 + 0.5*(a12+a21)*(a12+a21) + a22*a22;
	      state_new = GET_STATE((gradx*cos(phi)+grady*sin(phi)));
	  
	      t += dt;
	      if(t>T){break;}
	    }
	  log_file << "Episode " << episode << " state change " << endl;
	  //Calculating reward
	  delta_y = y - last_y;
