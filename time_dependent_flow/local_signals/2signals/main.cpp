  ...
  for(int episode=0 ; episode < episodes_number ; episode++)
    {
      ...
      a22 = interpolate(xmod,ymod,A22S,A,t_step);
      ...
      d2uy12 = interpolate(xmod,ymod,D2UY12S,A,t_step);
      d2uy22 = interpolate(xmod,ymod,D2UY22S,A,t_step);
...
{    
  ...
  for (int i = 0; i < n_fields; ++i)
    {
      g00 += f[i][x0_ind][y0_ind]*A[i][t+steady_A];
      g10 += f[i][x1_ind][y0_ind]*A[i][t+steady_A];
      g01 += f[i][x0_ind][y1_ind]*A[i][t+steady_A];
      g11 += f[i][x1_ind][y1_ind]*A[i][t+steady_A];
    }
  
  double fxy1 = (((d1x)/gridspace) * g00) + (((d2x)/gridspace)*g10);
  double fxy2 = (((d1x)/gridspace) * g01) + (((d2x)/gridspace)*g11);
