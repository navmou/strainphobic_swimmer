...
int main(){
  ...
  A = generate_As(sigma , tau , n_time_steps , dt);
  ...
  for(int episode=0 ; episode < episodes_number ; episode++)
    {
      ...
      log_file << "updated flows in " << elapsed_episode.count() << " s\n";
...
twoDvec generate_As(double sigma , double tau , int n_time_steps , double dt)
{
  twoDvec B = twoDzeros(n_fields, (n_time_steps+steady_A));
  for (int t = 0; t < (n_time_steps-1+steady_A); ++t)
    {
      for (int i = 0; i < n_fields; ++i)
	{
	  B[i][t+1] = B[i][t] - (B[i][t]/tau)*dt + sigma*dist(generator);
	}
    }
  return B;
}
...
void update_flow(int t) 
{
  ...
  for (int indx = 0; indx < field_x; ++indx)
      ...
      for (int indy = 0; indy < field_y; ++indy)
	{
	  for (int i = 0; i < n_fields; ++i)
	    {
	      UX[indx][indy] += UXS[i][indx][indy]*A[i][tt];
	      UY[indx][indy] += UYS[i][indx][indy]*A[i][tt];
	      A11[indx][indy] += A11S[i][indx][indy]*A[i][tt];
	      A12[indx][indy] += A12S[i][indx][indy]*A[i][tt];
	      A21[indx][indy] += A21S[i][indx][indy]*A[i][tt];
	      A22[indx][indy] += A22S[i][indx][indy]*A[i][tt];
	      D2UX11[indx][indy] += D2UX11S[i][indx][indy]*A[i][tt];
	      D2UX12[indx][indy] += D2UX12S[i][indx][indy]*A[i][tt];
	      D2UX22[indx][indy] += D2UX22S[i][indx][indy]*A[i][tt];
	      D2UY11[indx][indy] += D2UY11S[i][indx][indy]*A[i][tt];
	      D2UY12[indx][indy] += D2UY12S[i][indx][indy]*A[i][tt];
	      D2UY22[indx][indy] += D2UY22S[i][indx][indy]*A[i][tt];
	    }
	}
    }
}
