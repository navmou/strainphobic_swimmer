...
int GET_STATE(double gradx , double grady , double trSS);
...
const double grad_threshold_a = 1700.0;
const double grad_threshold_b = 10000.0;
const double trSS_threshold = 1.7;

int num_state = 50, num_action = 5;
...
beta 0.0 : {2,2,3,0,0,4,0,2,1,0,1,1,1,0,1,1,1,2,1,2,2,3,1,2,1,1,3,1,2,3,2,0,0,2,1,3,1,3,3,3,2,1,1,2,3,3,2,0,1,0};
beta 0.1 : {0,2,1,2,0,0,0,2,1,3,1,1,4,3,1,3,2,0,3,2,1,0,4,0,4,3,2,2,0,3,1,0,2,0,0,2,2,1,2,0,3,0,2,1,3,2,0,0,0,1};
beta 0.2 : {2,2,2,0,1,2,3,3,2,2,4,0,4,2,4,1,3,2,0,4,1,0,0,1,2,3,2,2,3,3,3,1,0,3,1,2,2,0,0,0,0,0,3,0,1,3,2,3,1,1};
beta 0.3 : {2,4,1,4,1,0,2,1,2,1,4,3,4,0,4,0,1,1,3,3,4,3,2,0,3,0,2,3,0,3,3,0,0,0,0,2,1,1,2,0,2,0,0,2,2,3,3,1,3,1};
beta 0.4 : {4,1,3,3,2,4,4,4,3,2,0,3,3,3,4,4,2,0,3,1,3,4,4,2,0,1,2,1,3,1,3,1,2,2,3,0,2,0,1,0,1,3,0,1,2,0,1,0,1,2};
beta 0.5 : {3,4,2,0,3,3,3,3,3,0,4,1,4,2,2,1,3,3,1,2,3,0,3,1,4,3,1,0,0,2,1,0,3,1,0,1,0,1,3,0,1,2,3,0,2,1,1,1,0,3};
beta 0.6 : {1,0,1,2,1,4,2,4,3,2,4,2,4,2,2,3,0,3,0,1,1,3,2,1,3,2,1,1,1,1,0,3,3,1,3,3,0,1,0,3,3,2,0,3,1,3,1,1,0,3};
beta 0.7 : {3,4,0,2,3,4,0,3,2,4,4,0,4,2,4,1,3,0,2,3,0,1,1,1,4,0,1,3,2,2,3,1,3,1,3,1,1,0,0,1,2,1,0,3,3,0,0,2,3,1};
beta 0.8 : {1,3,2,2,0,0,2,4,0,1,2,2,4,1,1,0,3,0,0,0,4,2,1,2,1,1,1,3,0,3,1,2,1,3,2,3,3,1,3,2,0,0,0,0,0,3,0,2,1,2};
beta 0.9 : {1,0,2,1,1,1,3,1,0,0,4,3,4,1,3,0,0,2,3,2,1,3,3,2,2,0,2,0,2,2,0,1,0,3,2,2,2,1,3,2,2,1,3,0,1,2,1,1,1,1};
beta 1.0 : {0,1,3,0,1,0,2,2,3,2,2,2,4,0,1,3,1,0,3,2,2,2,0,1,2,1,3,3,2,2,2,3,3,3,3,2,3,0,3,0,3,0,0,0,0,0,0,1,2,3};
*/
...
//opening HDF5 file
H5File file("random_gradxy.h5", H5F_ACC_TRUNC);
  ...
  for(int episode=0 ; episode < episodes_number ; episode++)
    {
      ...
      state = GET_STATE(gradx , grady , trSS);
      state_new = state;
	  ...
	  while(state_new == state)
	    {
	      ...
	      grady = 2*a11*d2ux12 + 2*a22*d2uy22 + (a12 + a21)*(d2ux22 + d2uy12);
	      trSS = a11*a11 + 0.5*(a12+a21)*(a12+a21) + a22*a22;
	      state_new = GET_STATE(gradx , grady , trSS);
...
//STATE EVALUATION
int GET_STATE(double grad_sx , double grad_sy , double TrSS){
  int state;
  if(TrSS < trSS_threshold)
    {
      if(grad_sx < -grad_threshold_b)
	{
	  if(grad_sy < -grad_threshold_b){state = 0;}
	  else if(grad_sy > grad_threshold_b){state = 4;}
	  else
	    {
	      if(grad_sy < -grad_threshold_a){state = 1;}
	      else if(grad_sy > grad_threshold_a){state = 3;}
	      else{state = 2;}
	    }
	}
      else if(grad_sx > grad_threshold_b)
	{
	  if(grad_sy < -grad_threshold_b){state = 20;}
	  else if(grad_sy > grad_threshold_b){state = 24;}
	  else
	    {
	      if(grad_sy < -grad_threshold_a){state = 21;}
	      else if(grad_sy > grad_threshold_a){state = 23;}
	      else{state = 22;}
	    }
	}
      else
	{
	  if(grad_sx < -grad_threshold_a)
	    {
	      if(grad_sy < -grad_threshold_b){state = 5;}
	      else if(grad_sy > grad_threshold_b){state = 9;}
	      else
		{
		  if(grad_sy < -grad_threshold_a){state = 6;}
		  else if(grad_sy > grad_threshold_a){state = 8;}
		  else{state = 7;}
		}
	    }
      
	  else if(grad_sx > grad_threshold_a)
	    {
	      if(grad_sy < -grad_threshold_b){state = 15;}
	      else if(grad_sy > grad_threshold_b){state = 19;}
	      else
		{
		  if(grad_sy < -grad_threshold_a){state = 16;}
		  else if(grad_sy > grad_threshold_a){state = 18;}
		  else{state = 17;}
		}
	    }
	    ...
	    {
	      if(grad_sy < -grad_threshold_b){state = 10;}
	      else if(grad_sy > grad_threshold_b){state = 14;}
	      else
		{
		  if(grad_sy < -grad_threshold_a){state = 11;}
		  else if(grad_sy > grad_threshold_a){state = 13;}
		  else{state = 12;}
		}
	    }
	}
  ...
  else
    {
      if(grad_sx < -grad_threshold_b)
	{
	  if(grad_sy < -grad_threshold_b){state = 25;}
	  else if(grad_sy > grad_threshold_b){state = 29;}
	  else
	    {
	      if(grad_sy < -grad_threshold_a){state = 26;}
	      else if(grad_sy > grad_threshold_a){state = 28;}
	      else{state = 27;}
	    }
	}
      else if(grad_sx > grad_threshold_b)
	{
	  if(grad_sy < -grad_threshold_b){state = 45;}
	  else if(grad_sy > grad_threshold_b){state = 49;}
	  else
	    {
	      if(grad_sy < -grad_threshold_a){state = 46;}
	      else if(grad_sy > grad_threshold_a){state = 48;}
	      else{state = 47;}
	    }
	}
      else
	{
	  if(grad_sx < -grad_threshold_a)
	    {
	      if(grad_sy < -grad_threshold_b){state = 30;}
	      else if(grad_sy > grad_threshold_b){state = 34;}
	      else
		{
		  if(grad_sy < -grad_threshold_a){state = 31;}
		  else if(grad_sy > grad_threshold_a){state = 33;}
		  else{state = 32;}
		}
	    }
	    ...
	    {
	      if(grad_sy < -grad_threshold_b){state = 40;}
	      else if(grad_sy > grad_threshold_b){state = 44;}
	      else
		{
		  if(grad_sy < -grad_threshold_a){state = 41;}
		  else if(grad_sy > grad_threshold_a){state = 43;}
		  else{state = 42;}
		}
	    }
	    ...
	    {
	      if(grad_sy < -grad_threshold_b){state = 35;}
	      else if(grad_sy > grad_threshold_b){state = 39;}
	      else
		{
		  if(grad_sy < -grad_threshold_a){state = 36;}
		  else if(grad_sy > grad_threshold_a){state = 38;}
		  else{state = 37;}
		}
	    }
	}
